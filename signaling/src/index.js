...
function validSession(session) {
  return /^[A-Za-z0-9_-]{10,40}$/.test(session || '');
}

async function generateTurnIceServers(env) {
  if (!env.TURN_KEY_ID || !env.TURN_KEY_API_TOKEN) {
    throw new Error('TURN is nog niet geconfigureerd');
  }
  ...
  const iceServers = (data.iceServers || []).map(server => {
    ...
    // Port 53 is known to time out in browsers; keep UDP/TCP/TLS fallbacks that are useful on iPad.
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/turn') {
      ...
      if (!validSession(session) || typeof key !== 'string' || key.length < 24 || key.length > 100) {
        return jsonError('Ongeldige sessie');
      }

      const id = env.SESSIONS.idFromName(session);
      const auth = await env.SESSIONS.get(id).fetch('https://internal/turn-auth', {
        method: 'POST',
      ...
      try {
        ...
        console.error(error);
        return jsonError('TURN tijdelijk niet beschikbaar', 503);
      }
    }
